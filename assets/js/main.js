const main = {
  ...
  bannerSlide: function () {
    ...
    if (banner) {
      const bannerItems = [
        ...document.querySelectorAll(".banner-slides__item"),
      ];
      ...
      function slide() {
        ...
        function runningSlide(index) {
          if (index < bannerItems.length - 1) {
            ...
            bannerItems[index].nextElementSibling.classList.add(
              "banner-slides__item--active"
            );
            dots[index].nextElementSibling.classList.add(
              "banner-slides__dot--active"
            );
          } else {
            for (let i = 0; i < bannerItems.length; i++) {
              bannerItems[i].classList.remove("banner-slides__item--active");
        ...
        function dotsClick() {
          dots.forEach((dot) => {
            dot.addEventListener("click", () => {
              ...
              let dotActive = document.querySelector(
                ".banner-slides__dot--active"
              );
              for (
                index;
