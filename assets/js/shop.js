...
function totalProduct(start, end, productsTotal) {
  ...
  totalProduct.innerText = `Showing ${
    start + 1
  } - ${end} of ${productsTotal} result`;
}
...
function renderButtonPagination(totalButtons) {
  ...
  for (let i = 0; i < totalButtons; i++) {
    ...
    numberPage.setAttribute(
      "class",
      "shop-pagination__list-item shop-pagination__list-number"
    );
...
function isButtonPagination() {
  let btnNumberActive = document.querySelector(
    ".shop-pagination__list-item--active"
  );
